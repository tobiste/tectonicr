...
- **Directional** data are $`2\pi`$-periodical, the angle $`\alpha`$ is
  ...
  (360°): $`\alpha = \alpha~\text{mod}~2\pi`$. Directional data usually
  involve processes where objects or particles were transported from one
  known place to another. Examples are wind direction, bird vanishing
  angles, plate motion, and fault slip directions.
- **Axial** data are $`\pi`$-periodical, i.e. each direction is
  ...
  the angle is $`\pi`$ (180°): $`\alpha = \alpha~\text{mod}~\pi`$. Data
  usually involves transport of objects where the origin is unknown or
  doesn’t matter as the transport could be extended infinitely. Examples
  are glacial striae, mineral alignments (long-axis of grain or crystal
