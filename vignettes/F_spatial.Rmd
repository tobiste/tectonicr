...
This vignette demonstrates some additional spatialy interpolated statisrtics of a stress field.
...
`circular_summary()` yields several statistics estimates for a given vector of angles, such as mean, median, standard deviation, quasi-quantiles, 95% confidence angle, as wells as the moments (2nd = variance, 3rd = skewness, 4th = kurtosis): 
...
Interpolated stress field color-coded by the skewness within each search window:
```{r}
...
ggplot(spatial_stats_comp) +
  ...
  geom_sf(data = trajectories, lty = 2) +
  geom_spoke(
    aes(lon, lat, angle = deg2rad(90 - mean), alpha = var),
    radius = .5, position = "center_spoke", lwd = .2, colour = "white"
  ) +
