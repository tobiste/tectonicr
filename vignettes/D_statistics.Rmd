...
```{r interpolation}
mean_SH <- stress2grid(san_andreas, gridsize = 1, R_range = seq(500, 800, 100))
```
...
constant no matter the distance between the data points. Assuming that the stress field is sourced by the plate boundary force, the model-based interpolation allows more reliable results for areas close to plate boundaries.

```{r interpolation_PoR}
mean_SH_PoR <- PoR_stress2grid(san_andreas, ep = euler, gridsize = 1, R_range = seq(500, 800, 100))
```
