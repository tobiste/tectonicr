...
```{r plot, echo=TRUE, warning=FALSE, message=FALSE, eval=TRUE}
...
the plate boundary:
```{r san.andreas.distanceplot1, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r san.andreas.distanceplot1_roll, warning=FALSE, message=FALSE, eval=TRUE}
san_andreas.res_roll <- san_andreas.res %>%
  arrange(distance) %>%
  mutate(
    roll_mean_azi.PoR = roll_circstats(azi.PoR, w = 1 / unc, FUN = circular_mean, width = 51)
  )

azi_plot +
  geom_step(
    data = san_andreas.res_roll,
    aes(distance, roll_mean_azi.PoR)
  )
```
...
```{r san.andreas.distanceplot2, warning=FALSE, message=FALSE, eval=TRUE}
nchisq_plot <- ggplot(san_andreas.res, aes(x = distance, y = nchisq)) +
...
Adding a rolling *Norm* $\chi^2$:
```{r san.andreas.distanceplot2_roll, warning=FALSE, message=FALSE, eval=TRUE}
nchisq_plot +
  geom_step(
    data = san_andreas.res_roll %>%
      mutate(roll_nchisq = roll_normchisq(azi.PoR, prd, unc, width = 51)),
