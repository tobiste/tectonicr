...
san_andreas.deviation <- deviation_shmax(san_andreas.prd, san_andreas$azi)
```
...
Also included are the plate boundary geometries after @Bird2003

```{r plates, echo=TRUE}
data("plates") # load plate boundary data set
```

Alternatively, there is also the NUVEL1 plate boundary model by @DeMets1990
...
and, if given, the deviation from the predicted $\sigma_{Hmax}$ direction 
...
ggplot(san_andreas.res, aes(x = distance, y = azi.PoR)) +
  ...
  labs(x = "Distance from plate boundary (B0)", y = "Azimuth in PoR (B0)") +
...
a strike-slip fault regime and are oriented around 135B0 wrt. to the Euler 
...
ggplot(san_andreas.res, aes(x = distance, y = nchisq)) +
  ...
  labs(x = "Distance from plate boundary (B0)", y = expression(Norm ~ chi^2)) +
...
beyond a distance of 4B0 and becomes random at ca. 8B0 away from the plate 
boundary. 
Thus, the North American-Pacific plate boundary zone at the San Andreas Fault is 
approx. 4--6B0 (ca. 440--670 km) wide. 
