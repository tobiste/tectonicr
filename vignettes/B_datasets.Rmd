...
trajectories <- eulerpole_loxodromes(euler, 40, cw = FALSE)
...
ggplot() +
  ...
  geom_spoke(
    data = as.data.frame(san_andreas.res),
    aes(
      x = lon,
      y = lat,
...
Adding a rolling statistics (e.g. weighted mean and sd.) of the transformed azimuth:
```{r san.andreas.distanceplot1_roll, warning=TRUE, message=TRUE, eval=TRUE}
san_andreas.res_roll <- san_andreas.res[order(san_andreas.res$distance), ] 

san_andreas.res_roll$roll_mean_azi.PoR <- roll_circstats(
