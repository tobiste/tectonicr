...
### Base R plot
The plot can also be build using base R's plotting engine:
...
Finally, we visualize the $\sigma_{Hmax}$ direction wrt. to the absolute distance to
the plate boundary:
```{r san.andreas.distanceplot1, echo=TRUE, message=FALSE, warning=FALSE}
azi_plot <- ggplot(san_andreas.res, aes(x = abs(distance), y = azi.PoR)) +
...
san_andreas.res_roll <- san_andreas.res[order(abs(san_andreas.res$distance)), ]
...
# plotting:
ggplot(san_andreas.res, aes(x = abs(distance), y = nchisq)) +
