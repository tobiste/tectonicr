...
## Mean orientation
Because orientation data is pi-periodical, i.e. the average of 35 degrees and 355 degrees should be 15 degree. 
`circular_mean()` and `circular_quasi_median()` are the metrics that express the average orientation:
...
```
## Inverse quality weighted mean orientation
Because the stress data is heteroscedastic, the data with less precise orientation should have less impact on the final mean orientation. The weighted mean or median uses the reported uncertainties of the measurements and weights the final average value by the inverse of the precision:
```{r weighted}
...
## Statistics in the Eule rpole reference frame
